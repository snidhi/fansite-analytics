...
# Read the input file 
             ...
             t_blocked = {}
             t = {}
             print time.asctime() + " Looping over input : " 
             for line in log:
		 try:
		 	ondash = line.split(' - - ')
                 	hostname = ondash[0]
...
# feature 3
             ...
             print time.asctime() + " Done Looping over input : " 
             time1 = abs_start
             while j1 <= len(timestamp_freq):
                ...
                if j2 <= len(timestamp_freq):
                    ...
                    for j in range(j1,len(timestamp_freq)):
                        hour_freq = hour_freq + timestamp_freq[j]
                j1 = j1+1                
                hashmap_hr_freq[reverse_convert_time(str(time1))] = hour_freq
                time1 = time1 + + datetime.timedelta(seconds=1)
                hour_freq =0
             
             print time.asctime() + " Sorting hostnames  : " 
             
             k = heapq.nlargest(10,hashmap_hostname.items(), key=lambda (k1,v1) : (v1))

             for word in k:
                 l = word[0]+','+str(word[1])+'\n'
                 f1.write(l)                 
             
             print time.asctime() + " Sorting resources  : " 
             k = heapq.nlargest(10, hashmap_resources.items(), key=lambda (k1,v1) : (v1))

             for word in k:
                 
                 f2.write(word[0]+'\n')
                 
             print time.asctime() + " Looping hr_freq    : " 
             s = heapq.nsmallest(10,hashmap_hr_freq.items(), key= lambda (k,v) :(-v,k))
